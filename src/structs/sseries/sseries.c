...
SSERIES *sseries_read_allocate(FILE *fp, SSERIES **head, SSERIES **curr, int type, char **token, int nnodes) {
    ...
    int id = get_next_token_int(token) - 1; // series ID
    ...
    for (ientry=0; ientry < s->size; ientry++) {
        ...
        get_token(line,token); sscanf((*token), "%lf", &s->entry[ientry].time);
        s->entry[ientry].time *= sdt_get_conversion_factor(in_units,TO);
        if (s->entry[ientry].time < s->entry[ientry - 1].time) {
    ...
    for (ivalue=0; ivalue < s->nvalues; ivalue++) {
        ...
        for (ientry=1; ientry < s->size; ientry++) {
            s->entry[ientry - 1].slope[ivalue] =
            (s->entry[ientry].value[ivalue] - s->entry[ientry - 1].value[ivalue]) /
            (s->entry[ientry].time - s->entry[ientry - 1].time);
        }
        // Calculate areas
        for (ientry=0; ientry < s->size-1; ientry++) {
            s->entry[ientry].area[ivalue] = tc_trap_area(s->entry[ientry].value[ivalue],
            s->entry[ientry +  1].value[ivalue], s->entry[ientry].time, s->entry[ientry + 1].time);
        }
    }
    
    //sseries_printScreen(*s, ON);
    
    if (type != DT_SERIES && type != OUTPUT_SERIES) {
        // if not a specialized list add to linked list
        sseries_add(s, head, curr, TRUE);
        sseries_free(s);
        //sseries_printScreen(**head, ON);
        //sseries_printScreen_list(ON,*head);
        return NULL;
    } else {
        return s;
    }
