...
int fe_sw2_body_resid(SMODEL_SUPER *mod, double *elem_rhs, int ie, double perturbation, int perturb_node, int perturb_var, int perturb_sign, int DEBUG) {
    ...
    double alpha = mod->ts->tau_temporal; //where to put this? this is specific to choice of time stepping scheme
...
//    }    
    ...
    if (is_double_small(elem_h_wd_avg) == NO) {        
        ...
        if (isTriangle == TRUE) {
            ...
            double y_coriolis = 0.0;// elem_vel_wd_avg.x * coriolis_speed;            // constant, strong equation set :: cjt :: trunk has factor on total stress and coriolis, but there are not wet-dry wrapped
